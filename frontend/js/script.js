// Get the HOST_IP environment variable
const HOST_IP = "localhost"; // Fallback to "localhost" if HOST_IP is not defined
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    fetch(`http://${HOST_IP}:8080/api/students`, {
      method: "POST",
      headers: {
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    let endpoint;
    if (searchType === "id") {
      endpoint = `http://${HOST_IP}:8080/api/students/${searchValue}`;
    } else {
      endpoint = `http://${HOST_IP}:8080/api/students/roll/${searchValue}`;
    }
    ...
    fetch(endpoint)
      ...
      .catch((error) => {
        ...
        if (error.message === "Student not found") {
          ...
          showNotification("Error finding student. Please try again.", "error");
        }
        console.error("Error:", error);
      });
  });
    ...
    .then((data) => {
      ...
      data.forEach((student) => {
        ...
        li.innerHTML = `
                  ...
                  <div class="student-detail">Marks: ${student.marks}</div>
                  <div class="student-detail">ID: ${student.id || "N/A"}</div>
              `;
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    const deleteSearchType = document.getElementById("deleteSearchType").value;
    ...
    if (deleteSearchType === "id") {
      endpoint = `http://${HOST_IP}:8080/api/students/${deleteSearchValue}`;
    } else {
      endpoint = `http://${HOST_IP}:8080/api/students/roll/${deleteSearchValue}`;
    }
      ...
      .then((student) => {
        ...
        document
          ...
          .addEventListener("click", function () {
            // Send DELETE request
            fetch(`http://${HOST_IP}:8080/api/students/${student.id}`, {
              method: "DELETE",
            })
              .then((response) => {
                if (!response.ok) {
  ...
  for (let i = 0; i < dots.length; i++) {
    ...
    if (mouse.x && mouse.y) {
      ...
      if (distToMouse < config.mouseInteractionRadius) {
        ...
        const sizeFactor = 1 - distToMouse / config.mouseInteractionRadius;
        dot.size =
          dot.baseSize +
        ...
        if (config.stableConnections) {
          ...
          dot.connections = dot.connections.filter((conn) => conn.opacity > 0);
        }
      }
    }
  }
...
document.addEventListener("touchmove", (event) => {
  if (event.touches.length > 0) {
    smoothMouseMovement(event.touches[0].clientX, event.touches[0].clientY);
